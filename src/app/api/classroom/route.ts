...
> 🔴 **答题 Slot 强制规则（前端渲染约束，必须严格遵守）**：
>
> 1. **每一个小问 (1)(2)(3)... 必须对应一个独立的答案输入框**，由前端逐题检测后动态渲染。前端会自动按 \`**(N)**\` 标记拆分出 N 个独立输入框，每框只收集该小问的答案。
> 2. **严禁多个小问合并进同一道 exercise**，除非它们确实是一个整体且只有一个答案。如有多个独立知识点的问题，拆开为多次 save_learning_resource 调用。
> 3. **禁止在题目 content 中出现"解题过程"/"计算过程"/"写出你的步骤"等字样**。前端不渲染计算过程文本框，只渲染每小问的最终答案输入框。学生只需填写每小问的最终答案（数字/字母/公式）。
> 4. 题目中不要加"请在下方答题区填写..."之类的提示语，前端会自动渲染对应数量的答案输入框。

**单问计算/简答题（只有一个答案）：**
\`\`\`
## [题干]

**题型：** [计算题/简答题]  **难度：** [简单/中等/困难]  **知识点：** [涉及知识点]
\`\`\`
